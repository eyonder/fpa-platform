...
import { useSalesPipelineForecastPreview } from "@/frontend/hooks/useSalesPipelineForecastPreview";
...
/**
 * Senaryo bazlı, TEKRAR ÇALIŞTIRILABİLİR bütçeye yazma — `SalesActualsPostPanel`
 * İLE AYNI iskelet: önizleme HİÇBİR ŞEY yazmaz, "Bütçeye Yaz" AÇIK (LEAD/
 * QUALIFIED/PROPOSAL/NEGOTIATION) TÜM fırsatların beklenen kapanış ayına
 * (expectedCloseDate) düşen, kazanma olasılığıyla AĞIRLIKLANDIRILMIŞ tutarını
 ...
 */
export function SalesPipelineForecastPostPanel() {
  const { state: scenariosState } = useScenarios();
  const [scenarioId, setScenarioId] = useState("");
  const { state: previewState, reload: reloadPreview } =
    useSalesPipelineForecastPreview(scenarioId || null);
  ...
  const commit = async () => {
    ...
    try {
      const result = await apiClient.post<BudgetLine[]>(
        "/sales-opportunities/pipeline-forecast-commit",
        { scenarioId },
      );
  ...
  return (
    <Card
      title="Boru Hattı Tahminini Bütçeye Yaz"
      hint="AÇIK fırsatların beklenen kapanış ayına düşen, kazanma olasılığıyla ağırlıklandırılmış tutarını 'Gelir' kategorisine yazar."
    >
        ...
        <div className="mt-4 space-y-4">
          {previewState.preview.monthlyTotals.length === 0 ? (
            <p className="text-sm text-muted">
              {previewState.preview.fiscalYear} mali yılına düşen açık fırsat yok (açık
              fırsat bulunamadı ya da beklenen kapanış tarihi bu yıla denk gelmiyor).
            </p>
          ) : (
            <>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-rule text-left text-xs text-muted">
                    ...
                    <th className="py-2 pr-3">Toplam Ağırlıklandırılmış Tutar</th>
                  </tr>
                </thead>
                <tbody>
                  {previewState.preview.monthlyTotals.map((t) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-3">
                        {formatAmount(t.totalWeighted, "TRY")}
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <ul className="tabular mt-2 space-y-1 text-xs">
                  {previewState.preview.opportunities.map((o) => (
                    <li key={o.opportunityId}>
                      {o.customerName} — {o.dealName} ({STAGE_LABEL[o.stage]}, %
                      {(o.winProbability * 100).toFixed(0)}): {o.month}. ay{" "}
                      {formatAmount(o.weightedAmount)}
                    </li>
                  ))}
                </ul>
              </details>
      ...
      <p className="mt-3 text-xs text-muted">
        Uyarı: bu ve &quot;Gerçekleşen Satışları Bütçeye Yaz&quot; işlemi AYNI
        kategoriye (&quot;Gelir&quot;) yazar. Farklı senaryo seçin — aynı senaryoyu
        ikisi için de kullanırsanız, ikinci yazma birincinin tutarlarının üzerine yazar.
      </p>
