...
import type { SalesActualsPreview } from "@/shared/types";

type State =
  | { status: "loading" }
  | { status: "ready"; preview: SalesActualsPreview }
  | { status: "error"; message: string };

function toMessage(error: unknown): string {
  return error instanceof ApiError
    ? error.message
    : "Actuals önizlemesi hesaplanamadı. Bağlantınızı kontrol edip tekrar deneyin.";
}

/** `useDepreciationPreview`teki AYNI desen: "loading" render sırasında TÜRETİLİR. */
export function useSalesActualsPreview(scenarioId: string | null) {
  const [result, setResult] = useState<{
    scenarioId: string;
    preview: SalesActualsPreview;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    apiClient
      .get<SalesActualsPreview>(
        `/sales-opportunities/actuals-preview?scenarioId=${encodeURIComponent(scenarioId)}`,
